...
OWNER_ID = int(os.environ.get("OWNER_ID", "6923501257"))
...
FRIENDS = [
    ...
    "ZENI","REX","ARNAV","TOXIC","OBITO","RAISEN","REXX",
...
def _eva_factory(txt: str, idx: int) -> callable:
    last = _last()
    fn   = _TOXIC_TMPL[idx % len(_TOXIC_TMPL)]
    def _f():
...
# ── Toxic Legend NCs (5 new) — PRIME ENGINE ─────────────────────────────────
...
async def cmd_evancs(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    def _cycling_factory():
        ...
        idx[0] += 1
        return f()
    await _start_nc(msg, cid, _cycling_factory, f"⚡ All Toxic NCs")
...
async def cmd_eva1(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    _nc_info[cid] = {"engine": "TOXIC1", "text": txt, "start_t": time.monotonic()}
...
async def cmd_evagod(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not bots:
        ...
        return
    gap = _nc_send_gap if _nc_send_gap is not None else 0.25
    _nc_info[cid] = {"engine": "TOXICGOD", "text": txt, "start_t": time.monotonic()}
...
async def cmd_ncwar(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not msg:
        return
    cid = msg.chat_id
    txt = _txt_arg(ctx) or "TOXIC GOD WAR"
...
async def _vc_get_entry(msg, args):
    ...
    # No name — use last saved, then first available
    if not lib:
        await _reply(msg, "⚠️ 𝐍𝐨 𝐬𝐨𝐧𝐠𝐬 𝐬𝐚𝐯𝐞𝐝! 𝐔𝐬?? -addsong <name> 𝐟𝐢𝐫𝐬𝐭.")
        return None, None
    last = _song_data.get("last", "")
...
async def main():
    ...
    print(f"[*] Starting {len(all_apps)} bots — TOXIC GOD")
    ...
    if _hard_stop:
        print("[✓] TOXIC GOD stopped.")
        return False       # caller exits

    print("[✓] TOXIC GOD restarting…")
